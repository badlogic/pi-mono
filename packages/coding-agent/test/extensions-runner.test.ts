...
describe("ExtensionRunner", () => {
	...
	describe("shortcut conflicts", () => {
		...
		it("warns but allows when extension uses non-reserved built-in shortcut", async () => {
			const pasteImageShortcut = Array.isArray(DEFAULT_KEYBINDINGS.pasteImage)
				? DEFAULT_KEYBINDINGS.pasteImage[0]
				: DEFAULT_KEYBINDINGS.pasteImage;
			const extCode = `
				export default function(pi) {
					pi.registerShortcut("${pasteImageShortcut}", {
			...
			expect(shortcuts.has(pasteImageShortcut.toLowerCase() as KeyId)).toBe(true);
