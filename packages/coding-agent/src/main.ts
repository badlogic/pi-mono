...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	return { command, source: sources[0], local, help, invalidOption };
}
	...
	try {
		if (options.command === "install") {
			...
			if (!source) {
				...
				console.error(chalk.dim(`Usage: ${getPackageCommandUsage("install")}`));
				process.exitCode = 1;
				return true;
			}
		...
		if (options.command === "remove") {
			...
			if (!source) {
				...
				console.error(chalk.dim(`Usage: ${getPackageCommandUsage("remove")}`));
				process.exitCode = 1;
				return true;
			}
