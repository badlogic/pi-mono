...
export async function loadCustomTools(
	...
	const sharedApi: ToolAPI = {
		...
		setEditorText: () => {},
		getEditorText: () => "",
		complete: async (_prompt: string, _options?: CompleteOptions) => {
			throw new Error("complete() is not available for custom tools");
		},
		showStatus: (message: string) => {
			if (sharedApi.hasUI) {
				sharedApi.ui.notify(message, "info");
			}
		},
		showError: (message: string) => {
			if (sharedApi.hasUI) {
				sharedApi.ui.notify(message, "error");
			}
		},
		copyToClipboard: () => {},
	};
