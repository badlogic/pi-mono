...
Add the tools directory to your PATH in \`~/.zshrc\` or \`~/.bashrc\`:

\`\`\`bash
export PATH="$HOME/agent-tools/${name}:$PATH"
\`\`\`
...
Or with full path (no PATH setup needed):
...
Generated by \`pi /wrap-mcp\` command, powered by [mcporter](https://github.com/steipete/mcporter).
`;
}
 ...
 * @param name - Package name
 * @param groups - Tool groups
 * @param packageDescription - Optional package description from registry
 * @returns AGENTS entry markdown
 */
export function generateAgentsEntry(name: string, groups: ToolGroup[], packageDescription?: string): string {
	...
	// Use package description from registry, fall back to generic
