...
7. `agent_settled` reports that automatic retries, compaction, and queued work have finished.
...
Event handlers run in extension load order. Some events are notifications; others can transform data, replace results, or cancel an operation. Use the event’s declared result type rather than assuming every return value has an effect.
