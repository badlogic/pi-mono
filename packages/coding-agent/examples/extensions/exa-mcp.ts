 ...
 *    - EXA_MCP_TOOLS (comma-separated list, appended to URL if tools param missing and limits registered tools)
...
class ExaMcpClient {
	...
	private async ensureInitialized(signal?: AbortSignal): Promise<void> {
		...
		if (!this.initializing) {
			this.initializing = this.initialize(endpoint, signal);
		}
...
export default function exaMcpExtension(pi: ExtensionAPI) {
	...
	const client = new ExaMcpClient(
		() => {
			...
			const baseUrl =
				typeof urlFlag === "string" ? urlFlag : (process.env.EXA_MCP_URL ?? config?.url ?? DEFAULT_ENDPOINT);
			const apiKey =
