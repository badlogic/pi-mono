...
describe("Generate E2E Tests", () => {
	...
	describe.skipIf(!process.env.DEEPSEEK_API_KEY)("DeepSeek Provider (deepseek-chat via OpenAI Completions)", () => {
	...
	describe.skipIf(!process.env.DEEPSEEK_API_KEY)(
		"DeepSeek Provider (deepseek-reasoner via OpenAI Completions)",
		() => {
			const llm = getModel("deepseek", "deepseek-reasoner");
	...
	describe.skipIf(!process.env.MOONSHOT_API_KEY)("Moonshot AI Provider (kimi-k2.5 via OpenAI Completions)", () => {
		const llm = getModel("moonshot", "kimi-k2.5");
