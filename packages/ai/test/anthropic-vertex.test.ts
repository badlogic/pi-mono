...
describe("Anthropic Vertex Provider", () => {
	...
	it.skipIf(!isVertexConfigured)("should complete basic text generation", { retry: 3, timeout: 30000 }, async () => {
	...
	it.skipIf(!isVertexConfigured)("should handle tool calling", { retry: 3, timeout: 30000 }, async () => {
		const context: Context = {
	...
	it.skipIf(!isVertexConfigured)("should handle streaming", { retry: 3, timeout: 30000 }, async () => {
		const context: Context = {
	...
	it.skipIf(!isVertexConfigured)("should handle multi-turn conversation", { retry: 3, timeout: 60000 }, async () => {
		const context: Context = {
			systemPrompt: "You are a helpful assistant. Be concise.",
			messages: [
				{
		...
		const text = response2.content.map((b) => (b.type === "text" ? b.text : "")).join("");
		expect(text.toLowerCase()).toContain("alice");
	});

	it.skipIf(!isVertexConfigured)("should report token usage", { retry: 3, timeout: 30000 }, async () => {
		const context: Context = {
			messages: [
				{
					role: "user",
		...
		expect(response.usage.totalTokens).toBe(
			response.usage.input + response.usage.output + response.usage.cacheRead + response.usage.cacheWrite,
		);
	});
});
...
describe("Anthropic Vertex Provider - Thinking (Opus 4.5)", () => {
	...
	it.skipIf(!isVertexConfigured)("should handle thinking blocks", { retry: 3, timeout: 120000 }, async () => {
		const context: Context = {
