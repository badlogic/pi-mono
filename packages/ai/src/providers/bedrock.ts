...
export const streamBedrock: StreamFunction<"anthropic-messages"> = (
	...
	(async () => {
		...
		try {
			const client = createBedrockClient(options?.interleavedThinking ?? true);
			const params = buildParams(model, context, options);
			const bedrockStream = client.messages.stream({ ...params, stream: true }, { signal: options?.signal });
...
function createBedrockClient(interleavedThinking: boolean): AnthropicBedrock {
	const awsRegion = process.env.AWS_REGION || "us-east-1";

	// AnthropicBedrock automatically picks up credentials from:
	// 1. AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY + AWS_SESSION_TOKEN (optional)
	// 2. AWS_PROFILE (via AWS SDK credential chain)
	// 3. EC2/ECS instance metadata
	// 4. Custom providerChainResolver
		...
		if (msg.role === "user") {
			...
			for (const block of msg.content) {
				if (block.type === "text") {
					...
					// If thinking signature is missing/empty, convert to text block
