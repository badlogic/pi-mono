...
async function generateModels() {
	...
	// Ensure every KnownProvider has an entry (even if empty) so that
	// TypeScript can index MODELS by KnownProvider.  Providers whose models
	// are registered at runtime via extensions still need a key here.
	for (const p of ["nebius"]) {
