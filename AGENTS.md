...
- NEVER run: `npm test`
- Do not run `npm run dev` or `npm run build` unless the user explicitly asks for a development server or production build.
- Exception: `packages/web-ui/example` may use `npm run dev` or `npm run build` when working on or serving the web UI example.
